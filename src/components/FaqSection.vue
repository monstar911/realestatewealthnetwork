<template>
  <section class="p-8 text-[#2f2f2f] text-left text-3xl">
    <div class="py-5 m-0 outline-none">
      <div class="px-[10px]">
        <div class="mt-0 outline-none cursor-pointer">
          <h1 class="text-center text-3xl">
            <b>Frequently Asked Questions...</b>
          </h1>
        </div>
        <div
          class="mt-[15px] outline-none cursor-pointer font-['Open_Sans',Helvetica,sans-serif]"
        >
          <h2 class="text-left text-xl font-bold">
            Is the content of this training brand new?
          </h2>
          <h3 class="text-left text-xl">
            <p class="leading-relaxed text-left">
              Absolutely. This is a brand new event and unlike anything I've
              ever done before. The focus of the event is 100% designed to get
              you comfortable doing deals as a virtual wholesaler.
            </p>
          </h3>
        </div>
        <div
          class="mt-[15px] outline-none cursor-pointer font-['Open_Sans',Helvetica,sans-serif]"
        >
          <h2 class="text-left text-xl font-bold">
            Will Cam actually be the one teaching?
          </h2>
          <h3 class="text-left text-xl">
            <p class="leading-relaxed text-left">
              YES. You're going to be spending 3 full days with me! I will have
              some members of my team helping out as well, but there are no
              outside speakers at all!
            </p>
          </h3>
        </div>

        <div
          class="mt-[15px] outline-none cursor-pointer font-['Open_Sans',Helvetica,sans-serif]"
        >
          <h2 class="text-left text-xl font-bold">
            Will you be going through the software tools?
          </h2>
          <h3 class="text-left text-xl">
            <p class="leading-relaxed text-left">
              YES. We plan to do multiple LIVE software demo's and walkthroughs
            </p>
          </h3>
        </div>

        <div
          class="mt-[15px] outline-none cursor-pointer font-['Open_Sans',Helvetica,sans-serif]"
        >
          <h2 class="text-left text-xl font-bold">
            Can Anyone Attend This Event?
          </h2>
          <h3 class="text-left text-xl">
            <p class="leading-relaxed text-left">
              YES. This training is appropriate for all levels of investors. If
              you have the desire to do more virtual wholesaling, then you're in
              the right place!
            </p>
          </h3>
        </div>

        <div
          class="mt-[15px] outline-none cursor-pointer font-['Open_Sans',Helvetica,sans-serif]"
        >
          <h2 class="text-left text-xl font-bold">
            Can I Get A Refund If I Can't Attend?
          </h2>
          <h3 class="text-left text-xl">
            <p class="leading-relaxed text-left">
              Seat deposit refunds may be given up to 14 days prior to the event
              start date as we have limited space for this training. However,
              within 14 days we can transfer your seat deposit to a future
              event.
            </p>
          </h3>
        </div>

        <div
          class="mt-[15px] outline-none cursor-pointer font-['Open_Sans',Helvetica,sans-serif]"
        >
          <h2 class="text-left text-xl font-bold">Any Other Questions?</h2>
          <h3 class="text-left text-xl">
            <p class="leading-relaxed text-left">
              Contact
              <a href="mailto:info@rewn.com" class="underline">info@rewn.com</a>
            </p>
          </h3>
        </div>
      </div>
      <CallToAction />
    </div>
  </section>
</template>

<script setup lang="ts">
import CallToAction from "./CallToAction.vue";
</script>
